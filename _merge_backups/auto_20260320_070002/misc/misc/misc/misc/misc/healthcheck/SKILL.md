...
Try to infer 1?? from the environment before asking. Prefer simple, non-technical questions if you need confirmation.
...
- ?œAre you using a Mac, Windows PC, or Linux???
- ?œAre you logged in directly on the machine, or connecting from another computer???
- ?œIs this machine reachable from the public internet, or only on your home/network???
- ?œDo you have backups enabled (e.g., Time Machine), and are they current???
- ?œIs disk encryption turned on (FileVault/BitLocker/LUKS)???
- ?œAre automatic security updates enabled???
- ?œHow do you use this machine???
  Examples:
...
1. ?œDo you want me to schedule periodic audits (e.g., daily/weekly) via `openclaw cron add`???
...
---
MERGED_FROM_BACKUP: C:\Users\wang\.openclaw\skills\misc\healthcheck\SKILL.md
---
---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for OpenClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, OpenClaw cron scheduling for periodic checks, or version status checks on a machine running OpenClaw (laptop, workstation, Pi, VPS).
...
- ?œAre you using a Mac, Windows PC, or Linux???- ?œAre you logged in directly on the machine, or connecting from another computer???- ?œIs this machine reachable from the public internet, or only on your home/network???- ?œDo you have backups enabled (e.g., Time Machine), and are they current???- ?œIs disk encryption turned on (FileVault/BitLocker/LUKS)???- ?œAre automatic security updates enabled???- ?œHow do you use this machine???  Examples:
  - Personal machine shared with the assistant
  - Dedicated local machine for the assistant
  - Dedicated remote machine/server accessed remotely (always on)
  - Something else?
